...
// Copyright (c) 2013 The K-COIN developers
...
uint256 hashGenesisBlock("0x0000058c44835e26c5a209bdb2ebfe324bcd8efa0b336c0444297531f9f7aa3c");
static const unsigned int timeGenesisBlock = 1437120784;
...
const string strMessageMagic = "K-COIN Signed Message:\n";
...
static const int64 nPremineRewardCoin = 8000000000 * COIN;
static const int64 nStartSubsidy = 1600 * COIN;
static const int64 nNewSubsidy = 0.1 * COIN;
static const int64 nMinSubsidy = 0 * COIN;
static const int64 nSubsidyHalvingInterval = 630720; //  Subsidy cut block

static const int64 nTargetTimespan = 10 * 60; // approx 10 minutes
static const int64 nTargetSpacing = 50; // 50 seconds block spacing
...
int64 static GetBlockValue(int nHeight, int64 nFees, unsigned int nBits)
{
    ...
    if (nHeight == 0) { nSubsidy = nGenesisBlockRewardCoin; } // Genesis Block Reward
    if (nHeight == 2) { nSubsidy = nPremineRewardCoin; } // Premine at blockheight 2
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x00000f891fde99bd286a5369801a3b0236bfe64095878982d6f796e17f5e93b0");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "Bigger Might Have Been Better for Goldman, JULY 16, 2015";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nBits    = bnProofOfWorkLimit.GetCompact();
				block.nNonce   = 2015994;
		
        if (fTestNet)
        {
            block.nTime    = 1437113875;
            block.nNonce   = 2054551;
        }
        ...
        block.print();
        assert(block.hashMerkleRoot == uint256("0x4c4da5afcd99dcbc0b4f812cf5f950779dfa352539f94f382ca0a06083009b3f"));
        // For Testing, please comment next line
...
unsigned char pchMessageStart[4] = { 0xfc, 0xb4, 0xd9, 0xac };
...
//        nBlockMaxSize = std::min(nBlockMaxSize, (unsigned int)(MAX_BLOCK_SIZE_GEN));
    ...
    int64 nFees = 0;
    {
        ...
        while (!vecPriority.empty())
        {
            ...
            // transactions:
            if (!fSortedByFee &&
                ((nBlockSize + nTxSize >= nBlockPrioritySize) || (dPriority < COIN * 1728 / 250)))
            {
                fSortedByFee = true;
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("K-COINMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("K-COINMiner : ProcessBlock, block not accepted");
    }
...
{
    printf("K-COINMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("k-coin-miner");
    ...
    try { loop {
        ...
        printf("Running K-COINMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
